...
class MeasuredData:
    ...
    def __init__(self, lightStr):
        self.lightConfig = self.loadConfig(lightStr)
        self.systemConfig = self.loadConfig("system_setup")
        self.exp = self.lightConfig["exposure"]
        self.src = self.systemConfig["src"]
        self.dst = self.systemConfig["dst"]
        self.mode = self.lightConfig["mode"]
    ...
    def Capture(self, mode = None, exp = None, config = None, loadBar = LoadBarLevel):
        """
    ...
    def beamMeasure(self, beamImg = None, configs = None, loadBar = LoadBarLevel):
        print("in beamMeasure()")
        if beamImg == None:
            beamImg = self.beamImg
        if configs == None:
            configs = [self.lightConfig, self.systemConfig]
        ...
        self.beamImg = res_img
        self.beamPf = pf
        self.symmetry = [cX, cY]
        self.size = [horiz_length, vert_length]
        self.boxMiddle = [midpoint_horizontal, midpoint_vertical]
        
        return rgbBeamImg, results

    def intensityMeasure(self, images = None, configs = None, loadBar = LoadBarLevel):
        if images == None:
            images = [self.intensityImg, self.beamImg]
        if configs == None:
            configs = [self.lightConfig, self.systemConfig]
        ## Parse Args ##
    ...
    def currentMeasure(self, lightConfig = None, loadBar = LoadBarLevel):
        ...
        if mode == 'M':
            ...
            # Current High Low
            c0Hi = lightConfig["pnp_high_current_good"] + lightConfig["pnp_high_current_tolerance"]
            c0Lo = lightConfig["pnp_high_current_good"] - lightConfig["pnp_high_current_tolerance"]
            loadBar.increase()
            c1Hi = lightConfig["npn_current_good"] + lightConfig["npn_current_tolerance"]
            c1Lo = lightConfig["npn_current_good"] - lightConfig["npn_current_tolerance"]
            loadBar.increase()
            c2Hi = lightConfig["pnp_low_current_good"] + lightConfig["pnp_low_current_tolerance"]
            c2Lo = lightConfig["pnp_low_current_good"] - lightConfig["pnp_low_current_tolerance"]
            loadBar.increase()
        ...
        try:
            if mode == 'M':
                ...
                if int(currentlist[0]) > c0Lo and int(currentlist[0]) < c0Hi:
                    pf[0] = True # NPN
                self.npnCurrent = currentlist[0]
                self.contPeak = currentlist[0]
                loadBar.increase()
                if int(currentlist[1]) > c1Lo and int(currentlist[1]) < c1Hi:
                    pf[1] = True # PNP hi
                self.pnpHiCurrent = currentlist[1]
                self.analogHi = currentlist[1]
                loadBar.increase()
                if int(currentlist[2]) > c2Lo and int(currentlist[2]) < c2Hi:
                    pf[2] = True # PNP lo
                self.pnpLoCurrent = currentlist[2]
                self.analogLo = currentlist[2]
                loadBar.increase()
